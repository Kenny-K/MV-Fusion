...
class RangeFireUp(nn.Module):
    def __init__(self, inplanes, squeeze_planes,
                expand1x1_planes, expand3x3_planes, bn_d, stride):
        super(RangeFireUp, self).__init__()
        self.inplanes = inplanes
        self.bn_d = bn_d
        self.stride = stride
        self.activation = nn.ReLU(inplace=True)
        ...
        if self.stride == 2:
            ...
            self.expand1x1 = nn.Conv2d(squeeze_planes, expand1x1_planes,
                                    kernel_size=1)
            self.expand1x1_bn = nn.BatchNorm2d(expand1x1_planes, momentum=self.bn_d)
            self.expand3x3 = RangeConv2d(squeeze_planes, expand3x3_planes,
                                    kernel_size=3, padding=1)
            self.expand3x3_bn = nn.BatchNorm2d(expand3x3_planes, momentum=self.bn_d)
    ...
    def forward(self, x, depth):
        ...
        return torch.cat([
            self.activation(self.expand1x1_bn(self.expand1x1(x))),
            self.activation(self.expand3x3_bn(self.expand3x3(x, depth)))
        ], 1)
...
class RangeFire(nn.Module):
    def __init__(self, inplanes, squeeze_planes,
                expand1x1_planes, expand3x3_planes, bn_d=0.1):
        super(RangeFire, self).__init__()
        self.inplanes = inplanes
        self.bn_d = bn_d
        self.activation = nn.ReLU(inplace=True)
        self.squeeze = nn.Conv2d(inplanes, squeeze_planes, kernel_size=1)
...
class Conv2(nn.Module):
    def __init__(self, stride, bn_d) -> None:
        super().__init__()
        self.strides = stride
        self.bn_d = bn_d
        self.pool = nn.MaxPool2d(kernel_size=3,
                                stride=[1, self.strides[1]],
                                padding=1)
        self.fire1 = RangeFire(64, 16, 64, 64, bn_d=self.bn_d)
        self.cam1 = CAM(128, bn_d=self.bn_d)
        self.fire2 = RangeFire(128, 16, 64, 64, bn_d=self.bn_d)
        self.cam2 = CAM(128, bn_d=self.bn_d)

    def forward(self, x, depth):
        x = self.pool(x)
        x = self.fire1(x, depth)
        x = self.cam1(x)
        x = self.fire2(x, depth)
        x = self.cam2(x)
        return x
...
class Conv3(nn.Module):
    def __init__(self, stride, bn_d) -> None:
        ...
        self.pool = nn.MaxPool2d(kernel_size=3,
                                stride=[1, self.strides[2]],
                                padding=1)
        self.fire1 = RangeFire(128, 32, 128, 128, bn_d=self.bn_d)
        self.fire2 = RangeFire(256, 32, 128, 128, bn_d=self.bn_d)
...
class Conv4(nn.Module):
    def __init__(self, stride, bn_d) -> None:
        ...
        self.pool = nn.MaxPool2d(kernel_size=3,
                                stride=[1, self.strides[3]],
                                padding=1)
        self.fire1 = RangeFire(256, 48, 192, 192, bn_d=self.bn_d)
        self.fire2 = RangeFire(384, 48, 192, 192, bn_d=self.bn_d)
        self.fire3 = RangeFire(384, 64, 256, 256, bn_d=self.bn_d)
        self.fire4 = RangeFire(512, 64, 256, 256, bn_d=self.bn_d)

    def forward(self, x, depth):
        x = self.pool(x)
        x = self.fire1(x, depth)
        x = self.fire2(x, depth)
        x = self.fire3(x, depth)
        x = self.fire4(x, depth)
        return x

class RangeConvNet(nn.Module):
    def __init__(self, cfg) -> None:
        super().__init__()
        self.bn_d = 0.1
        self.input_depth = 5
        ...
        self.conv1b = nn.Sequential(nn.Conv2d(self.input_depth, 64, kernel_size=1,
                                          stride=1, padding=0),
                                nn.BatchNorm2d(64, momentum=self.bn_d))
        self.conv2 = Conv2(self.strides, self.bn_d)
        self.conv3 = Conv3(self.strides, self.bn_d)
        self.conv4 = Conv4(self.strides, self.bn_d)

        self.upconv1 = RangeFireUp(512, 64, 128, 128, bn_d=self.bn_d,
                                stride=self.strides[0])
        self.upconv2 = RangeFireUp(256, 32, 64, 64, bn_d=self.bn_d,
                                stride=self.strides[1])
        self.upconv3 = RangeFireUp(128, 16, 32, 32, bn_d=self.bn_d,
                                stride=self.strides[2])
        self.upconv4 = RangeFireUp(64, 16, 32, 32, bn_d=self.bn_d,
                                stride=self.strides[3])

        self.FC = nn.Conv2d(64,self.out_dim,kernel_size=1)

        self.squeeze_map = nn.AvgPool2d(kernel_size=3, stride=[1,2], padding=1)

    
    def forward(self, x):
        # Convolutional Encoder
        depth = x[:,0,:,:]      # depth b x 64 x 1024
        depth_2 = self.squeeze_map(depth)   # b x 64 x 512
        skip_1 = self.conv1b(x).detach()    # b x 64 x 64 x 1024
        skip_2 = self.conv1a(x)             # b x 64 x 64 x 512

        depth_4 = self.squeeze_map(depth_2) # b x 64 x 256
        skip_3 = self.conv2(skip_2, depth_4)    # b x 64 x 64 x 256
        skip_2 = skip_2.detach()
        
        depth_8 = self.squeeze_map(depth_4) # b x 64 x 128
        skip_4 = self.conv3(skip_3, depth_8)         # b x 64 x 64 x 128
        skip_3 = skip_3.detach()

        depth_16 = self.squeeze_map(depth_8) # b x 64 x 64
        code = self.conv4(skip_4, depth_16)           # b x 64 x 64 x 64
        skip_4 = skip_4.detach()
        ...
        # Convolutional Decoder
        out = self.upconv1(code,depth_8) + skip_4
        out = self.upconv2(out,depth_4) + skip_3
        out = self.upconv3(out,depth_2) + skip_2
        out = self.upconv4(out,depth) + skip_1
...
class RangeConv2d(nn.Module):
    def __init__(self, in_plane=3, out_plane=16, kernel_size=3, padding=1) -> None:
        super().__init__()
        self.conv1 = nn.Conv2d(in_channels=in_plane, out_channels=out_plane, kernel_size=kernel_size, padding=padding)
        self.norm1 = nn.BatchNorm2d(num_features=out_plane)
    ...
    def forward(self, x, depth):
        assert x.shape[-2:] == depth.shape[-2:], print('Feature img and depth img must have the same size.')
        depth = depth / torch.max(depth, dim=0).values
        depth = torch.unsqueeze(depth,1)
        xd = x*depth
        x1 = self.conv(x) * depth
        xd = self.conv(xd)
        x = self.conv(x)
        return x - torch.abs(x1 - xd)
...
if __name__ == '__main__':
    test_img = torch.randn(2,3,1024,64)
    depth = torch.rand(2,1024,64) * 20
    rangconv = RangeConv2d(3,64,3)
    res = rangconv(test_img, depth)
    print(res.shape)
